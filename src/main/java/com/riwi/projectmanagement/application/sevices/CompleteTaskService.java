...
/**
 * Service - Implementa el caso de uso "Completar Tarea"
 */
@Service
public class CompleteTaskService implements CompleteTaskUseCase {
    ...
    public Task execute(CompleteTaskCommand command) {
        ...
        // 4. Verificar que el usuario es el propietario del proyecto (Regla de negocio)
        if (!project.isOwnedBy(currentUserId)) {
            throw new IllegalStateException(
                    "Solo el propietario del proyecto puede completar tareas"
            );
        }

        // 5. Completar la tarea (aplica regla de negocio: no puede estar ya completada)
        // El método complete() del dominio valida la regla
        task.complete();
        ...
        auditLogPort.register("TASK_COMPLETED", completedTask.getId());

        // 8. Enviar notificación
        notificationPort.notify(
                "Tarea '" + completedTask.getTitle() + "' ha sido completada"
        );

        // 9. Devolver la tarea completada
